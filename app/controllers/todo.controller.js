...
exports.create = (req, res) => {
    ...
    if (!req.body.description) {
        return res.status(400).send({
            message: "Descrição do Todo não pode ser vazio"
        });
    }

    // Create a Todo
    const todo = new Todo({
        name: req.body.name || "Sem título Todo",
    ...
    todo.save()
        .then(data => {
            ...
            res.status(500).send({
                message: err.message || "Ocorreu algum erro ao criar o Todo."
            });
        });
};
...
exports.findAll = (req, res) => {
    Todo.find()
        .then(todos => {
            ...
            res.status(500).send({
                message: err.message || "Ocorreu algum erro ao tentar recuperar todos."
            });
        });
};
